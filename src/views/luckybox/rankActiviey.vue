    ...
    <div class="page-box" :class="['step' + step]">
      ...
      <div class="top-box">
        ...
        <template v-if="(step === 2)">
          <div class="wait-title fx-vc">
            <div class="btxt" :data-text="$t('lucky.waitingResults')">
              <span>
                {{ $t('lucky.waitingResults') }}
              </span>
            </div>
          </div>
            ...
            <div class="oh">
              <div class="the-users" :class="{ 'avatar-step1': (step === 1)}">
                <div class="ou" :class="['ou-' + idx]" v-for="(ou, idx) in topList" :key="idx">
                  ...
                  <div class="name" v-if="ou.countryId" :style="{'--mainImg': ('url('+ 'https://h5.livuchat.com/static/images/countryFlag/' + ($countryCode[[0, 1, 2, 3, 4, 5].includes(ou.countryId) ? ou.countryId = 6 : ou.countryId].short.toLowerCase()) + '.png' +')')}">{{ ou.userName }}</div>
                  <div class="name" v-else>{{ ou.userName }}</div>
                  <div style="display: flex; flex-direction: column;">
                    <div class="won"><span class="won-span">{{$t('cargame.twon')}}:</span>{{ ou.coin | formatNumberDot }}</div>
      ...
      <div class="help-pop" v-if="showHelp">
        ...
        <div class="p-content">
          ...
          <div class="p-body">
            <p v-html="$t('lucky.rules1', { time1: '<br>' + rstime, time2: rltime })"></p>
            <p v-html="$t('lucky.rules2', { time2: '<br>' + rltime, time3: retime })"></p>
            <p>{{ $t('lucky.rules3',{ box_name: $t('christmas2022.boxName')}) }}</p>
            <p>{{ $t('lucky.rules4') }}</p>
            <p>{{ $t('lucky.rules5') }}</p>
            <p>{{ $t('lucky.rules6') }}</p>
...
import { apiGetStarConf, apiGetStarCycle, apiGetStarRank, apiGetStarHistoryList, apiGetStarHistoryDetail, apiGetStarTop, apiGetStarHistoryTop } from '@/api/rankActiviey'
...
export default {
  ...
  watch: {
    ...
    isBtnHelp (n) {
      ...
      if (n) {
        triggerService({ eventId: '105-14-2-2', freeName1: freeName[this.step] })
      }
    }
  },
  computed: {
...
.page-box {
  min-height: 100%;
  background: url(../../assets/img/star/star-bg-5.png) center top no-repeat;
  ...
  &.step2 {
    background: url(../../assets/img/star/lucky-bg-2.png) center top no-repeat;
    background-size: 100% auto;
  }
  &.step3 {
    background: url(../../assets/img/star/lucky-bg-3.png) center top no-repeat;
    background-size: 100% auto;
    .top-box {
      height: 450px;
    }
...
.time-box {
  ...
  .otime {
    width: 52px;
    height: 60px;
    background: url(../../assets/img/star/lucky-dao.png) center center no-repeat;
    background-size: auto 100%;
    text-align: center;
    color: #fff;
    margin: 0 2px;
...
.top-three {
  ...
  .op {
    ...
    width: 156px;
    height: 30px;
    background: linear-gradient(90deg, #FEE566 0%, rgba(254,232,105,0) 100%);
    border-radius: 21px;
    margin: 0 2px;
...
.rank-box {
  ...
  .h-list {
    .oh {
      ...
      height: 180px;
      margin: 0 auto 20px;
      background: url(../../assets/img/star/lucky-top-bg.png) center top no-repeat;
...
.help-pop {
  ...
  .p-content {
    width: 320px;
    height: 430px;
    background: linear-gradient(180deg, #FFDF9D 0%, #FFA61F 19%, #CE861F 51%, #D18922 100%);
    border-radius: 30px;
    // border: 3px solid;
    // backdrop-filter: blur(2px);
    position: absolute;

<template>
  <div :class="['page-bonus', showFlip && 'horizontal-flip']">
      <div class="top-info">
        ...
        <div class="title">
          <p>{{$t('christmas2022.bonusTitle3')}}</p>
      ...
      <div v-if="!activeEnd" class="bonus-list">
      ...
      <mt-popup
        ...
        >
        <div class="rules">
          <div class="rule-title"> {{ $t("christmas2022.bonusRule3") }} <span class="back-btn closeRule" @click="closeRule">
            <img src="~@/assets/img/newYear/close-bg1.png" alt="">
          </span></div>
          <div class="rule-box">
  ...
  methods: {
    ...
    async getRechargeBonus (item) {
      ...
      if (item.bonus > 0 && item.bonusEnable) {
        ...
        // }
        const res = await getUserRechargeBonus({ ...commonParams, stage: item.stage })
        ...
        parent.postMessage({ type: 'getCion' }, '*')
        this.getList()

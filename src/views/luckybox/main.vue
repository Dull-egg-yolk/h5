<template>
  <div :class="[showFlip && 'horizontal-flip']">
    <div :class="['page-lucky']" v-if="isLoad">
      ...
      <div v-if="isLoad && actLifeCycle === ACT_LIFECYCLE.running && stage && stage === 1" ref="scrollRef">
        <div class="free-wrap">
          <div class="free-box">
            <div class="gift-pkg-box">
              <img :class="[isShowOpenAni ? 'show': '']" src="https://h5.livuchat.com/staticProd/20230109_luckybox/img/22222.png" alt="">
            </div>
            ...
            <div :class="['gift-wrap', isShowOpenAni ? 'show': '']" @click="openRuleBox">
              <ul class="gift" v-if="ruleMap && collectionInfo">
                <li :class="['type', goodsDictMap[g] && collectionInfo.selfLeftCollecttedGoodsIdCountMap[g] && (goodsDictMap[g].indexOf('6.png') !== -1 ||  goodsDictMap[g].indexOf('5.png') !== -1) ? 'special': '']" v-for="(g) in Object.keys(ruleMap)" :key="g">
                  <img :src="goodsDictMap[g]"  v-if="collectionInfo.selfLeftCollecttedGoodsIdCountMap[g]" />
                  <img
                    class="cover"
                    src="https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/00.png"
          ...
          <div
            ...
            >
              <div class="button" v-if="endBtnTime && endBtnTime <= Date.now() " @click.stop="openFreeGift">
                {{ $t('christmas2022.freeBoxName', {box_name: $t('christmas2022.boxName') }) }}
              </div>
              <div class="cover-btn" v-if="!isShowFreeBtn && endBtnTime > Date.now()">
                ...
                <p> {{ $t('christmas2022.freeBoxName', {box_name: $t('christmas2022.boxName') }) }}</p>
              </div>
            </div>
        </div>
        <!-- 收集阶段付费宝箱 -->
        <div class="pay-box-wrap" v-if="isLoad &&actLifeCycle === ACT_LIFECYCLE.running &&stage === STAGE.collect">
          <div class="info">
            ...
            <p>{{ $t('christmas2022.actInfo', {total: multiLangParams.shareCoin, items_name:  $t('christmas2022.itemName'), box_name: $t('christmas2022.boxName')}) }}</p>
          </div>
          <div class="pay-box-list">
            <div class="item" v-for="(p, idx) in payedList" :key="p.id">
              <div class="type-img"><img :src="lotteryDict[p.id]" /></div>
              <div class="left-tips">{{ $t('christmas2022.boxTopInfo', {items_name:  $t('christmas2022.itemName')}) }}</div>
              <div class="right-tips">
                  <div><i>{{ $t('worldCup.winUp') }}</i></div>
                  <div><span>{{ p.maxValuePerOnce | formatNum }}</span></div>
              </div>
              <div class="gift-wrap-pay">
                <ul class="gift-small-pay" v-if="Object.keys(ruleMap).length > 0">
                  <li :class="['type1', goodsDictMap[g] && (goodsDictMap[g].indexOf('6.png') !== -1 ||  goodsDictMap[g].indexOf('5.png') !== -1) ? 'special': '' ? 'special': '']" v-for="g in Object.keys(ruleMap)" :key="g">
                    <img :src="goodsDictMap[g]" />
      ...
      <div v-if="actLifeCycle === ACT_LIFECYCLE.end || !stage || stage !==1" :class="['act-end']" >
        <div class="info">{{ $t('worldCup.worldCupBonus.actEnd') }}</div>
      </div>
      <jump-bonus @jumpBonus="jumpBonus" v-if=" (!(commonParams && commonParams.platformType && commonParams.platformType === '4'))&& isLoad"></jump-bonus>
    </div>
      ...
      <div class="rules">
        <div class="rule-title">
          ...
          <span class="back-btn closeRule" @click="closeRule">
            <img src="https://h5.livuchat.com/staticProd/20230109_luckybox/img/close.png" alt="">
          </span>
        </div>
        <div class="rule-box">
          <dl>
            ...
            <dt>{{$t('christmas2022.rule1', {total: multiLangParams.shareCoin, items_name:  $t('christmas2022.itemName'), box_name: $t('christmas2022.boxName')})}}</dt>
            <dt>{{$t('christmas2022.rule2', {hour: 12, items_name:  $t('christmas2022.itemName'), box_name: $t('christmas2022.boxName')})}}</dt>
            <dt>{{$t('christmas2022.rule3', {hour: 12, items_name:  $t('christmas2022.itemName'), box_name: $t('christmas2022.boxName')})}}</dt>
            <dt>
              <span v-html="$t('christmas2022.rule4', {
                lotteryIcon: multiLangParams.lotteryIcon,
                cup: multiLangParams.cup,
                count: multiLangParams.count
              })"></span>
            </dt>
            <dt>{{$t('christmas2022.rule5')}}</dt>
          </dl>
        </div>
      </div>
    </mt-popup>
    ...
    >
      <div class="gift-wrap-2">
        ...
        <div class="gift-box" v-if="selGift && goodsDictMap">
            ...
            <div class="info">
              <div class="item-wrap">
                 <span :class="['item', selGift.right < 100 ? 'gift1': selGift.right >= 100 && selGift.right < 500 ? 'gift2': 'gift3']">
...
const aniImgs = [
  'https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/ani/box1.png',
  'https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/ani/box02.png',
  'https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/ani/box03.png',
  'https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/ani/box04.png',
  'https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/ani/box05.png',
  'https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/ani/coins.png',
...
export default {
  ...
  data () {
    return {
      ...
      multiLangParams: {
        total: '10,000,00',
        lotteryIcon: '<img src="https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/box05.png" class="dimond">',
        cup: '<img src="https://h5.livuchat.com/staticProd/20230109_luckybox/img/gift/06.png">',
        count: '90',
        shareCoin: '200,000'
      },
      isShowAni: false,
  ...
  methods: {
    ...
    goPayment () {
      if (this.commonParams.platformType !== '4') {
